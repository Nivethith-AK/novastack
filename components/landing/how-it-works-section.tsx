...
const steps = [
  ...
  },
  {
    ...
    code: `novastack.workflow('process-orders', {
  trigger: 'new_order',
...
function highlightCode(line: string): string {
  // Escape HTML first so regex replacements only operate on plain text.
  const escaped = line
    .replace(/&/g, "&amp;")
    .replace(/</g, "&lt;")
    .replace(/>/g, "&gt;");

  // Apply highlighting in a deterministic order to avoid nested-tag corruption.
  return escaped
    .replace(/(novastack|process|env)/g, '<span class="text-foreground">$1</span>')
