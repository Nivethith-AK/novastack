...
const codeExamples = [
  {
    label: "Initialize",
    code: `import { NovaStack } from '@novastack/sdk'

const novastack = new NovaStack({
  apiKey: process.env.NOVASTACK_API_KEY
})`,
  },
  {
    label: "Inference",
    code: `const response = await novastack.inference({
  model: 'novastack-3-turbo',
  input: userMessage,
  stream: true
})
...
for await (const chunk of response) {
  ...
  {
    label: "Batch",
    code: `const batch = await novastack.batch.create({
  model: 'novastack-3',
  inputs: documents,
            ...
            <div className="rounded-xl overflow-hidden bg-card border border-border card-shadow">
              ...
              <div className="border-t border-border p-4 bg-secondary/20">
                <div className="flex items-center gap-2 text-xs font-mono text-muted-foreground mb-2">
                  ...
                  <span>npm install @novastack/sdk</span>
                </div>
